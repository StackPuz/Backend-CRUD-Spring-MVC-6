            ...
            <div class="row">
                <div class="col">
                    <div class="col-12"><input id="searchbar_toggle" type="checkbox" />
                        <div id="searchbar" class="form-row mb-4">
                            <div class="form-group col-lg-2">
                                <select id="search_col" onchange="searchChange()" class="form-control form-control-sm">
                                    <option value="id" data-type="number" ${param.sc=='id' ? 'selected' : '' }>Product Id</option>
                                    <option value="image" ${param.sc=='image' ? 'selected' : '' }>Product Image</option>
                                    <option value="name" ${param.sc=='name' ? 'selected' : '' }>Product Name</option>
                                    <option value="price" data-type="number" ${param.sc=='price' ? 'selected' : '' }>Product Price</option>
                                    <option value="brand.name" ${param.sc=='brand.name' ? 'selected' : '' }>Brand Name</option>
                                    <option value="userAccount.name" ${param.sc=='userAccount.name' ? 'selected' : '' }>User Account Name</option>
                                </select>
                            </div>
                            <div class="form-group col-lg-2">
                        ...
                        <table class="table table-sm table-striped table-hover">
                            <thead>
                                <tr>
                                    ...
                                    <th class="${util.getSortClass('image')} d-none d-md-table-cell"><a href="${util.getLink('sort',paging,'image')}">Image</a></th>
                                    <th class="${util.getSortClass('name')}"><a href="${util.getLink('sort',paging,'name')}">Name</a></th>
                                    <th class="${util.getSortClass('price')} d-none d-md-table-cell"><a href="${util.getLink('sort',paging,'price')}">Price</a></th>
                                    <th class="${util.getSortClass('brand.name')} d-none d-md-table-cell"><a href="${util.getLink('sort',paging,'brand.name')}">Brand</a></th>
                                    <th class="${util.getSortClass('userAccount.name')} d-none d-md-table-cell"><a href="${util.getLink('sort',paging,'userAccount.name')}">Create User</a></th>
                                    <th>Actions</th>
                                </tr>
                            </thead>
                            <tbody>
                                <c:forEach var="product" items="${products}">
                                    <tr>
                                        ...
                                        <td class="d-none d-md-table-cell text-center">
                                            <c:if test="${product.image != null}"> <a href="${pageContext.request.contextPath}/uploads/products/${product.image}" target="_blank" title="${product.image}"><img class="img-list" src="${pageContext.request.contextPath}/uploads/products/${product.image}" /></a> </c:if>
                                        </td>
                                        <td>${product.name}</td>
                                        <td class="d-none d-md-table-cell text-right">${product.price}</td>
                                        <td class="d-none d-md-table-cell">${product.brand.name}</td>
                                        <td class="d-none d-md-table-cell">${product.userAccount.name}</td>
                                        <td class="text-center">
                                            <a class="btn btn-sm btn-secondary" href="${pageContext.request.contextPath}/products/${product.id}" title="View"><i class="fa fa-eye"></i></a>
                                            <a class="btn btn-sm btn-primary" href="${pageContext.request.contextPath}/products/edit/${product.id}" title="Edit"><i class="fa fa-pencil"></i></a>
                                            <a class="btn btn-sm btn-danger" href="${pageContext.request.contextPath}/products/delete/${product.id}" title="Delete"><i class="fa fa-times"></i></a>
                                        </td>
                                    </tr>
                                </c:forEach>
                            </tbody>
                        ...
                        <a class="btn btn-sm btn-primary" href="${pageContext.request.contextPath}/products/create">Create</a>
                    </div>
                    <style>
